...
class Brightsky extends utils.Adapter {
    ...
    private async onReady(): Promise<void> {
        ...
        this.log.info(
            `Adapter started with configuration: Position: ${this.config.position}, WMO Station ID: ${this.config.wmo_station}, DWD Station ID: ${this.config.dwd_station_id}, ${this.config.createCurrently ? `Currently data enabled. Poll interval: ${this.config.pollIntervalCurrently} minutes` : 'Currently data disabled'} - ${this.config.createDaily ? `Daily forecast: ${this.config.forecastDays} days, hourly for ${this.config.hourlyForecastDays} days. Poll interval: ${this.config.pollInterval} hours` : ''} - ${this.config.createDaily ? (this.config.createDailyCustomInterval ? `Daily data enabled. Poll interval: ${this.config.createDailyCustomInterval}h` : 'Daily data enabled') : 'Daily data disabled'} - ${this.config.createRadar ? `Radar data enabled. Poll interval: ${this.config.pollIntervalRadar} minutes` : 'Radar data disabled'}. Max distance: ${this.config.maxDistance} meters.`,
        );
        this.log.info(
    ...
    async weatherDailyLoop(): Promise<void> {
        ...
        if (!this.config.createDailyCustomInterval) {
            ...
            loopTime = new Date().setHours(
                Math.min(new Date().getHours() + this.config.createDailyCustomInterval, 24),
                0,
                0,
                0,
            );
        ...
        try {
            ...
            if (result.data) {
                ...
                if (result.data.weather && Array.isArray(result.data.weather)) {
                    ...
                    const rawHourlyByDay: BrightskyWeather[][] = [];
     ...
     */
    private calculateApparentTemperature(
                ...
                case 'apparent_temperature': {
                    ...
                    if (values && values.length > 0) {
                        ...
                        if (k === 'apparent_temperature') {
                            const min = Math.min(...(values.filter(v => v !== null) as number[]));
                            const max = Math.max(...(values.filter(v => v !== null) as number[]));
                            (result as any).apparent_temperature_min =
                                min !== Infinity ? Math.round(min * 10) / 10 : null;
                            (result as any).apparent_temperature_max =
                                max !== -Infinity ? Math.round(max * 10) / 10 : null;
                            (result as any).apparent_temperature_median =
                                median !== null ? Math.round(median * 10) / 10 : null;
                            result.apparent_temperature = avg as number | null; // Add average value
                        } else {
                        ...
                        if (k === 'apparent_temperature') {
                            (result as any).apparent_temperature_min = null;
                            (result as any).apparent_temperature_max = null;
                            (result as any).apparent_temperature_median = null;
