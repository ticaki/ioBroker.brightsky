...
class Brightsky extends utils.Adapter {
    ...
    rawRadarData = [];
    radarRotationTimeout = undefined;
    /**
    ...
    constructor(options = {}) {
        ...
        this.library = new library_1.Library(this, 'Brightsky');
    }
    /**
    ...
    async onReady() {
        ...
        if (!this.config.createDaily) {
            ...
            await this.library.writedp('daily', null, definition_1.genericStateObjects.weather.daily._channel);
        ...
        if (!this.config.createCurrently) {
            ...
            await this.library.writedp('current', null, definition_1.genericStateObjects.weather.current._channel);
        }
        ...
        if (!this.config.createRadar) {
            ...
            await this.library.writedp('radar', null, definition_1.genericStateObjects.weather.radar._channel);
            await this.library.writedp(`radar.max_precipitation_forecast`, null, definition_1.genericStateObjects.max_precipitation_forecast._channel);
        ...
        if (!this.config.createCurrently &&
            ...
            this.log.error('No data creation is enabled in the adapter configuration. Please enable at least one of the options: Currently, Hourly, Daily, or Radar.');
            return;
        }
        ...
        if (this.config.wmo_station !== '' && this.config.dwd_station_id !== '') {
            this.log.warn('Both WMO station ID and DWD station ID are set. Using DWD station ID for location identification.');
        ...
        if (this.config.hourlyForecastDays == undefined || this.config.hourlyForecastDays < 0 || this.config.hourlyForecastDays > this.config.forecastDays) {
            this.config.hourlyForecastDays = Math.min(3, this.config.forecastDays);
        }
        if (this.config.pollInterval == undefined ||
        ...
        if (this.config.pollIntervalCurrently == undefined ||
            this.config.pollIntervalCurrently < 10 ||
            this.config.pollIntervalCurrently >= 2 ** 31 / 60000) {
            this.log.warn(`Invalid poll interval currently: ${this.config.pollIntervalCurrently}. Using default value of 30 minute.`);
        ...
        if (this.config.pollIntervalRadar == undefined ||
            this.config.pollIntervalRadar < 5 ||
            this.config.pollIntervalRadar >= 2 ** 31 / 60000) {
            this.log.warn(`Invalid poll interval radar: ${this.config.pollIntervalRadar}. Using default value of 10 minutes.`);
        ...
        if (this.config.pollIntervalRadar % 5 !== 0) {
            ...
            this.log.warn(`Radar poll interval must be divisible by 5. Adjusting from ${this.config.pollIntervalRadar} to ${adjusted} minutes.`);
            this.config.pollIntervalRadar = adjusted;
        }
        // Validate radar distance (max 50km)
        if (this.config.radarDistance == undefined ||
            this.config.radarDistance < 400 ||
            this.config.radarDistance > 50000) {
            this.log.warn(`Invalid radar distance: ${this.config.radarDistance}. Using default value of 2000 meters (2 km).`);
            this.config.radarDistance = 2000; // Default to 2 km if invalid
        }
        if (this.config.createCurrently) {
        ...
        this.log.info(`Adapter started with configuration: Position: ${this.config.position}, WMO Station ID: ${this.config.wmo_station}, DWD Station ID: ${this.config.dwd_station_id}, ${this.config.createCurrently ? `Currently data enabled. Poll interval: ${this.config.pollIntervalCurrently} minutes` : 'Currently data disabled'} - ${this.config.createDaily ? `Daily forecast: ${this.config.forecastDays} days, hourly for ${this.config.hourlyForecastDays} days. Poll interval: ${this.config.pollInterval} hours` : ''} - ${this.config.createDaily ? (this.config.createDailyCustomInterval ? `Daily data enabled. Poll interval: ${this.config.createDailyCustomInterval}h` : 'Daily data enabled') : 'Daily data disabled'} - ${this.config.createRadar ? `Radar data enabled. Poll interval: ${this.config.pollIntervalRadar} minutes` : 'Radar data disabled'}. Max distance: ${this.config.maxDistance} meters.`);
        this.log.info(`Using ${this.config.dwd_station_id ? `WMO Station ID: ${this.config.dwd_station_id}` : `${this.config.wmo_station ? `WMO Station ID: ${this.config.wmo_station}` : `Position: ${this.config.position} with max distance: ${this.config.maxDistance} meters`}`}`);
    }
    /**
    ...
    async weatherDailyLoop() {
        ...
        if (!this.config.createDailyCustomInterval) {
            if (new Date().getHours() >= 5 && new Date().getHours() < 18) {
                loopTime = new Date().setHours(18, 0, 0, 0);
            }
            else if (new Date().getHours() >= 18) {
                loopTime = new Date().setHours(0, 0, 0, 0);
            }
            else {
                loopTime = new Date().setHours(5, 0, 0, 0);
            }
        }
        else {
            loopTime = new Date().setHours(Math.min(new Date().getHours() + this.config.createDailyCustomInterval, 24), 0, 0, 0);
        }
        loopTime = loopTime + 30000 + Math.ceil(Math.random() * 30000) - Date.now(); // Calculate the time until the next update
        if (loopTime <= 0) {
    ...
    async weatherDailyUpdate() {
        ...
        try {
            const response = await this.fetch(`https://api.brightsky.dev/weather?${this.posId}&max_dist=${this.config.maxDistance}&date=${startTime}&last_date=${endTime}`);
            this.log.debug(`https://api.brightsky.dev/weather?lat=${this.config.position.split(',')[0]}&lon=${this.config.position.split(',')[1]}&max_dist=${this.config.maxDistance}&date=${startTime}&last_date=${endTime}`);
            ...
            if (result.data) {
                ...
                if (result.data.weather && Array.isArray(result.data.weather)) {
                    ...
                    for (const item of result.data.weather) {
                        ...
                        // Calculate apparent temperature for each hourly item
                        item.apparent_temperature = this.calculateApparentTemperature(item.temperature, item.wind_speed, item.relative_humidity);
                        const day = Math.floor((new Date(item.timestamp).getTime() - currentDay) / (24 * 60 * 60 * 1000));
                    ...
                    for (let i = 0; i < weatherArr.length; i++) {
                        ...
                        for (const key of Object.keys(weatherArr[i])) {
                            ...
                            switch (k) {
                                ...
                                case 'temperature': {
                                    ...
                                    if (values && values.length > 0) {
                                        ...
                                        if (k !== 'solar') {
                                            ...
                                            if (this.config.position.split(',').length === 2 &&
                                                ...
                                                dailyData.solar_estimate = values.reduce((sum, value, index) => {
                                                    ...
                                                    if (value) {
                                                        const newValue = this.estimatePVEnergyForHour(value, new Date(weatherArr[i].timestamp[index]), {
                                ...
                                case 'apparent_temperature': {
                                    ...
                                    if (values && values.length > 0) {
                                        if (values && values.length > 0) {
                                            ...
                                            // For apparent_temperature, also calculate min/max like temperature
                                            if (k === 'apparent_temperature') {
                                                const min = Math.min(...values.filter(v => v !== null));
                                                const max = Math.max(...values.filter(v => v !== null));
                                                dailyData.apparent_temperature_min =
                                                    min !== Infinity ? Math.round(min * 10) / 10 : null;
                                                dailyData.apparent_temperature_max =
                                                    max !== -Infinity ? Math.round(max * 10) / 10 : null;
                                                dailyData.apparent_temperature_median =
                                                    median !== null ? Math.round(median * 10) / 10 : null;
                                                dailyData.apparent_temperature = avg; // Add average daily value
                                            }
                                            else {
                                            ...
                                            if (k === 'apparent_temperature') {
                                                ...
                                                dailyData.apparent_temperature = null; // Add average daily value
                                            }
                                            else {
                                                dailyData[k] = null;
                                ...
                                case 'condition': {
                                    ...
                                    dailyData.conditionUI = this.library.getTranslation(condition.charAt(0).toUpperCase() + condition.slice(1));
                                    break;
                                }
                                default: {
                        ...
                        const times = suncalc.getTimes(targetDate, parseFloat(this.config.position.split(',')[0]), parseFloat(this.config.position.split(',')[1]));
                    ...
                    await this.library.writeFromJson('daily.r', 'weather.daily', definition_1.genericStateObjects, resultArr, true);
    ...
    handleFetchError(error) {
        if (error.name !== 'AbortError') {
            ...
            if (error instanceof Error) {
                ...
                isHttpError =
                    isHttpError || error.message.includes('HTTP') || error.status || error.url;
    ...
    async weatherCurrentlyUpdate() {
        try {
            const response = await this.fetch(`https://api.brightsky.dev/current_weather?${this.posId}&max_dist=${this.config.maxDistance}`);
            if (response.status !== 200) {
            ...
            if (result.data) {
                ...
                if (result.data.weather) {
                    ...
                    weather.apparent_temperature = this.calculateApparentTemperature(weather.temperature, weather.wind_speed_10, weather.relative_humidity);
                    await this.library.writeFromJson('current', 'weather.current', definition_1.genericStateObjects, weather, true);
                    await this.library.writedp('current.sources', undefined, definition_1.genericStateObjects.weather.sources._channel);
                    await this.library.writeFromJson('current.sources.r', 'weather.sources', definition_1.genericStateObjects, result.data.sources, true);
                    await this.setState('info.connection', true, true);
                }
            }
        }
    ...
    async weatherRadarUpdate() {
        try {
            ...
            const response = await this.fetch(`https://api.brightsky.dev/radar?lat=${coords[0]}&lon=${coords[1]}&distance=${this.config.radarDistance}&date=${dateParam}&format=plain`);
            if (response.status !== 200) {
    ...
    async writeRadarData() {
        ...
        if (this.config.createRadarData) {
            ...
            for (let i = 0; i < this.radarData.length; i++) {
                ...
                // Use a zero-padded label for nicer sorting/labels (e.g., "00", "05", "10", ...)
            ...
            if (dataToWrite.length > 0) {
                await this.library.writeFromJson('radar.data.r', 'weather.radar', definition_1.genericStateObjects, dataToWrite, true);
            }
        }
    ...
    async writeMaxPrecipitationForecasts() {
        ...
        for (const [key, value] of Object.entries(forecasts)) {
            await this.library.writedp(`radar.max_precipitation_forecast.${key}`, value, definition_1.genericStateObjects.max_precipitation_forecast[key]);
        }
                ...
                case 'apparent_temperature': {
                    ...
                    if (values && values.length > 0) {
                        ...
                        if (k === 'apparent_temperature') {
                            const min = Math.min(...values.filter(v => v !== null));
                            const max = Math.max(...values.filter(v => v !== null));
                            result.apparent_temperature_min =
                                min !== Infinity ? Math.round(min * 10) / 10 : null;
                            result.apparent_temperature_max =
                                max !== -Infinity ? Math.round(max * 10) / 10 : null;
                            result.apparent_temperature_median =
                                median !== null ? Math.round(median * 10) / 10 : null;
                            result.apparent_temperature = avg; // Add average value
                        }
                        else {
                        ...
                        if (k === 'apparent_temperature') {
                            ...
                            result.apparent_temperature = null; // Add average value
                        }
                        else {
                            result[k] = null;
    ...
    estimatePVEnergyForHour(valueWhPerM2, time, coords, panels) {
        ...
        for (let i = 0; i < 4; i++) {
            const quarterHourTime = time instanceof Date
    ...
    estimatePvEnergy(valueWhPerM2, time, coords, panels, wrArray) {
        ...
        const normEff = (pct) => clamp01(pct / 100); // 0..100% → 0..1
