...
Drop-in Snippet, in `<adapter>` den eigenen Adapternamen einsetzen
(z.B. `brightsky`). Port 9230 ist frei wählbar, muss aber in beiden Settings
identisch sein.
    ...
    "configurations": [
        {
            ...
            "type": "node",
            "request": "launch",
            "program": "${workspaceFolder}/.dev-server/default/node_modules/iobroker.<adapter>/build/main.js",
            "args": ["--debug", "0"],
            ...
            "sourceMaps": true,
            "outFiles": [
                "${workspaceFolder}/.dev-server/default/node_modules/iobroker.<adapter>/build/**/*.js"
            ],
            "resolveSourceMapLocations": [
                "${workspaceFolder}/**",
                "${workspaceFolder}/.dev-server/default/node_modules/iobroker.<adapter>/build/**"
            ]
        }
    ],
    "version": "0.2.0"
...
Wenn der Adapter ein anderes `main`-File hat (selten — meist `build/main.js`),
muss der `program`/`outFiles`-Pfad angepasst werden. Maßgeblich ist
`package.json#main`.
...
>     `node node_modules/iobroker.<adapter>/build/main.js --debug 0`
> from within `<profileDir>`

Das `cwd` ist also `${workspaceFolder}/.dev-server/default` (nicht
`.../node_modules/iobroker.<adapter>`). Aus diesem cwd findet der Adapter
das js-controller-Setup, die Redis-Ports der Profile-Instanz usw.
...
`.dev-server/default/node_modules/iobroker.<adapter>/build/` — also genau in
einem `node_modules`-Pfad. Mit Default-Excludes wird die Sourcemap dort gar
nicht erst geladen → grauer Breakpoint.
...
```
/usr/bin/node --experimental-network-inspection ./node_modules/iobroker.<adapter>/build/main.js --debug 0
```
...
1. **Sourcemap auflösen**: stimmt der absolute Pfad?
   ...
   node -e '
     ...
     const p = ".dev-server/default/node_modules/iobroker.<adapter>/build/main.js.map";
...
2. **Source-Drift**: stimmt `sourcesContent` exakt mit dem Workspace-File überein?
   ...
   node -e '
     const fs = require("fs");
     const m = JSON.parse(fs.readFileSync(".dev-server/default/node_modules/iobroker.<adapter>/build/main.js.map", "utf8"));
...
2. dev-server synct in `.dev-server/default/node_modules/iobroker.<adapter>/build/`
...
nodemon und `--inspect`. Dann statt Launch-Config die "Attach to remote"-Config
(Port 9229) verwenden. Nachteil: Initial-Pause via `--inspect-brk` geht nicht;
dafür Auto-Reload.

## Übertragung auf andere Adapter

1. Datei kopieren, `<adapter>` im launch.json-Snippet ersetzen.
2. Falls `package.json#main` nicht `build/main.js` ist, Pfade entsprechend anpassen.
3. Falls Port 9230 belegt: andere freie Portnummer in beiden Settings
   (`attachSimplePort` und `runtimeArgs`) konsistent setzen. js-controller im
   dev-server nutzt 9228, Default-Inspect ist 9229 — also alles ≥ 9230 ist sicher.
4. `agent/`-Ordner aus npm-Pack ausschließen: `package.json#files` ist eine
   Allowlist — `agent/` einfach nicht mit aufnehmen. Aus Git wird er nur
   ausgeschlossen, wenn `.gitignore` Dot-Dirs whitelistet (`.*/`); ein
   Top-Level-`agent/` ist automatisch im Git enthalten.
